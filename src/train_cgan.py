...
eae = EAE(image_size=image_size, image_channel=image_channel, std_channel=std_channel, latent_dim=latent_dim, num_class=num_class, norm='bn').to(device)
eae.apply(initialize_weights)

g = Generator(image_size=image_size, image_channel=image_channel, std_channel=std_channel, latent_dim=latent_dim, num_class=num_class, norm='bn').to(device)
d = Discriminator(image_size=image_size, image_channel=image_channel, std_channel=std_channel, latent_dim=latent_dim, num_class=num_class).to(device)
...
eae_optimizer = Adam(eae.parameters(),
                     lr=learning_rate,
                     betas=(beta1, beta2))
mse_loss = nn.MSELoss()
...
with torch.no_grad():
    eae.eval()
    fixed_vector_output = eae(fixed_image[target_idx].to(device), fixed_label[target_idx].to(device))
...
# init G model
g.load_state_dict(eae.state_dict(), strict=False)
d.load_state_dict(eae.state_dict(), strict=False)
