...
def stack_gen_data_features(eval_model, gen_model, latent_dim, sample_size, num_class, resizer, device):
    ...
    for cls in range(num_class):
        for idx in range(sample_size//10):
            # print(idx)
...
if __name__ == "__main__":
    ...
    eval_model = InceptionV3(resize_input=False, normalize_input=False)
    ...
    for epoch in range(0, 100, 2):
        gen_model = Generator(image_size=64, image_channel=3, std_channel=64, latent_dim=128, num_class=10, norm='bn')
        gen_model.load_state_dict(torch.load(f'/home/sin/git/ae/src/weights/evae/ebgan_cifar10/g_{epoch}.pth'))
        real_image_features, real_labels = stack_real_data_features(eval_model, train_loader, resizer, device=cuda0)
        gen_image_features, gen_labels = stack_gen_data_features(eval_model, gen_model, latent_dim, sample_size, num_class, resizer, device=cuda1)

        result_epoch = f"{epoch}\t"
        for cls in range(num_class):
            # print(cls)
