...
# load generator
# gen_path = '/home/sin/git/clone/improved-bagan-gp/codes/bagan_gp_cells_v3_2_epoch100.h5'
gen_path = '/home/sin/git/clone/improved-bagan-gp/codes/weights/bagan_gp_generator(fashion_mnist).h5'
...
real = np.ndarray(shape=(images.shape[0], 64, 64))
for i in range(images.shape[0]):
    # real[i] = cv2.resize(images[i], (64, 64)).reshape((64, 64, channel))
    real[i] = cv2.resize(images[i], (64, 64))

# Train test split, for autoencoder (actually, this step is redundant if we already have test set)
real_imgs, x_test, real_label, y_test = train_test_split(real, labels, test_size=0.3, shuffle=True, random_state=42)
