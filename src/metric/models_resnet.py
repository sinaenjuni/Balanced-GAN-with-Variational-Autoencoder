...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ori = F.interpolate(ori, scale_factor=2, mode="nearest")
        ori = self.conv2d0(ori)
        out = x + ori
        return out
...
class Decoder(nn.Module):
    def __init__(self, z_dim, g_shared_dim, img_size, g_conv_dim, apply_attn, attn_g_loc, g_cond_mtd, num_classes, g_init, g_depth,
        ...
        self.MODEL = MODEL
        self.in_dims = in_dims[str(img_size)]
        self.num_blocks = len(self.in_dims)

        self.out_dims = out_dims[str(img_size)]
        ...
        for index in range(self.num_blocks):
            self.blocks += [[
                DecoderBlock(in_channels=self.in_dims[index],
